...
**MinuteFrame**은 Google Meet, Zoom 같은 화상회의와 일반 화면 작업을 저장하는 macOS/Windows 데스크톱 녹화 앱입니다. 화면 또는 창, 시스템 오디오, 마이크를 한 번에 녹화하고, 장시간 녹화에서도 메모리가 계속 늘지 않도록 녹화 데이터를 즉시 디스크에 기록합니다.

현재 버전은 **0.2.4**입니다. Electron, React, TypeScript로 사용자 화면을 구성하고, 운영체제별 네이티브 모듈을 함께 사용해 시스템 오디오와 macOS 창 커서 정책을 처리합니다.

## 이 앱이 해결하는 문제

화상회의를 녹화할 때는 발표 화면뿐 아니라 상대방의 음성과 내 마이크를 함께 담아야 합니다. MinuteFrame은 화면 캡처, 시스템 오디오, 마이크, 파일 저장을 하나의 흐름으로 묶고 다음 원칙을 따릅니다.
